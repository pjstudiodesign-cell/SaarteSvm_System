...
def iniciar_conexao():
    ...
    if not url or not key:
        st.error("Erro: Chaves de conexão não encontradas no Render.")[cite: 1]
        return None
...
def gerar_pdf_orcamento(cliente, servico, valor, pgto, prazo, rev, obs):
    try:
        ...
        pdf.cell(0, 12, "SaarteSvm", ln=True, align='C')[cite: 1]
        pdf.set_font("Arial", 'I', 10)
        pdf.set_text_color(255, 255, 255)
        pdf.cell(0, 6, str(info[1]), ln=True, align='C')[cite: 1]
        pdf.set_font("Arial", '', 9)
        pdf.set_text_color(200, 200, 200)
        pdf.cell(0, 6, f"WhatsApp: {info[2]} | Email: {info[3]}", ln=True, align='C')[cite: 1]
        if info[4]: pdf.multi_cell(0, 5, f"Endereço: {info[4]}", align='C')[cite: 1]
        pdf.set_y(75)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(100, 10, f"CLIENTE: {str(cliente).upper()}", ln=0)[cite: 1]
        pdf.cell(0, 10, f"DATA: {datetime.now().strftime('%d/%m/%Y')}", ln=1, align='R')[cite: 1]
        pdf.ln(10); pdf.set_font("Arial", 'B', 14)
        pdf.cell(0, 10, "1. DESCRICAO DO SERVICO", ln=True)[cite: 1]
        pdf.set_font("Arial", '', 11); pdf.multi_cell(0, 7, f"{servico}")[cite: 1]
        if obs:
            pdf.ln(2); pdf.set_font("Arial", 'B', 11); pdf.cell(10, 7, "Obs: ")[cite: 1]
            pdf.set_font("Arial", '', 11); pdf.multi_cell(0, 7, f"{obs}")[cite: 1]
        pdf.ln(5); pdf.set_font("Arial", 'B', 14); pdf.cell(0, 10, "2. CONDICOES", ln=True)[cite: 1]
        pdf.set_font("Arial", '', 11)
        pdf.cell(0, 8, f"- Prazo: {prazo} | Revisões: {rev}", ln=True)[cite: 1]
        pdf.cell(0, 8, f"- Forma de Pagamento: {pgto}", ln=True)[cite: 1]
        pdf.set_y(-40); pdf.set_font("Arial", 'B', 18)
        pdf.cell(0, 15, f"INVESTIMENTO TOTAL: R$ {valor:,.2f}", ln=True, align='R')[cite: 1]
...
def gerar_pdf_recibo(cliente, servico, valor):
    try:
        ...
        pdf.cell(0, 15, "RECIBO DE PAGAMENTO", ln=True, align='C')[cite: 1]
        pdf.ln(10); pdf.set_font("Arial", '', 12)
        texto = f"Recebemos de {str(cliente).upper()}, a importância de R$ {valor:,.2f} referente ao serviço de: {servico}."[cite: 1]
        pdf.multi_cell(0, 10, texto, align='L')[cite: 1]
        pdf.ln(10); pdf.cell(0, 10, f"Data: {datetime.now().strftime('%d/%m/%Y')}", ln=True, align='R')[cite: 1]
        pdf.ln(15); pdf.cell(0, 10, "__________________________________________________", ln=True, align='C')[cite: 1]
        pdf.set_font("Arial", 'B', 10); pdf.cell(0, 5, "SaarteSvm", ln=True, align='C')[cite: 1]
        return pdf.output(dest='S').encode('latin-1', 'ignore')
    except: return None[cite: 1]
...
def main():
    ...
    if escolha == "Painel":
        ...
        with st.form("orc_form"):
            c1, c2, c3 = st.columns(3) # Ajustado para 3 colunas para incluir CPF/CNPJ
            n = c1.text_input("Cliente")[cite: 1]
            tel = c2.text_input("WhatsApp")[cite: 1]
            doc = c3.text_input("CPF/CNPJ") # CAMPO ADICIONADO
            
            end_cli = st.text_input("Endereço Completo") # CAMPO ADICIONADO
            ...
            c4, c5, c6 = st.columns(3); prz = c4.text_input("Prazo", "10 dias úteis")[cite: 1]
            rev = c5.selectbox("Revisões", ["Padrão", "1", "2", "3", "Ilimitadas"])[cite: 1]
            pag = c6.text_input("Pagamento", "50% entrada / 50% entrega")[cite: 1]
            if st.form_submit_button("SALVAR"):
                if n and ser:
                    # Salvando na tabela exclusiva SaarteSvm - Adicionados novos campos ao dicionário
                    dados = {
