...
# 2. Conexão Blindada com Supabase
@st.cache_resource
def iniciar_conexao():
...
# 3. Estilo Visual Premium (Lacrado)
def aplicar_estilo():
    st.markdown("""
        <style>
...
def gerar_documento_pdf(tipo, cliente, servico, valor, doc_id="", data_extra=""):
    try:
        ...
        pdf.set_y(-30); pdf.set_font("Arial", 'I', 8); pdf.set_text_color(100, 100, 100)
    ...
    if escolha == "Painel":
        ...
        with st.form("orc_form"):
            ...
            if st.form_submit_button("GERAR E SALVAR"):
                ...
                else: st.error("Campos obrigatórios faltando.")
        ...
        for r in res.data:
            with st.expander(f"📝 EDITAR: {r['cliente']} | ID: {r['id']}"):
                with st.form(f"edit_{r['id']}"):
                    ...
                    if col_btn2.form_submit_button("EXCLUIR PROJETO"):
